...
let verifyCode = {
    ...
    bindAllInputEventByInput : function () {
        for (let i = 0; i < this.inputCount; i++) {
            let that = this
            this.inputArray[i].oninput = function (event) {
                return that.eventByOninput(that,event,this)
            }
        }
    },
    ...
    eventByOninput:function(that,event,elem){
        let data = event.data
        if(data==undefined || data == null){
            ...
            setTimeout(function () {
                ...
                if(data.length == that.inputCount && that.isNumbersByStr(data)){
                    that.clearAllValue()
                    that.setValue(data)
                    that.ifExecuteSend(that)
                }else{
    ...
    setValue : function(value){
        for (let i = 0; i < this.inputCount; i++) {
            if(i > value.length-1){
                this.nextInput( this.inputArray[i-1])
                return
            }
            this.inputArray[i].value = value[i]
        }
    },
